...
instance Functor f => Functor (StateT s f) where
  (<$>) ::
    ...
    -> StateT s f a
    -> StateT s f b
  (<$>) g s = StateT $ (applyFst g <$>) . runStateT s

  -- runState s :: s -> f (a, s)
  -- applyFst g :: (a, c) -> (b, c)
  -- applyFst g <$> :: f (a, c) -> f (b, c)
...
instance Bind f => Apply (StateT s f) where
  (<*>) ::
    ...
    -> StateT s f a
    -> StateT s f b
  (<*>) mf st = StateT $ \s -> runStateT mf s >>= \(f, s') -> applyFst f <$> runStateT st s'
...
distinctF ::
  ...
  List a
  -> Optional (List a)
distinctF xs = fst <$> runStateT ( filtering p xs ) S.empty
    where p x = StateT $ \s -> if x <= 100 then Full ( not $ S.member x s, S.insert x s ) else Empty
